...
botoesGatilho.forEach(botao => {
    botao.addEventListener('click', (evento) => {
        ...
        const menuDesejado = idDoMenu ? getElement(idDoMenu) : null;
...
const form = getElement('createJobForm');
...
async function populateCityOptions(state, selectedCity = '') {
    ...
    if (window.__ifCampiByUF && window.__ifCampiByUF[state]) {
        window.__ifCampiByUF[state].forEach((c) => { if (!cities.includes(c))
            cities.push(c); });
    }
    cities.sort();
    cities.forEach((city) => {
...
function buildJobPayload() {
    ...
    return {
        ...
        target: (getElement('regTarget')?.value ?? ''),
...
function populateJobForm(job) {
    if (editJobIdInput)
        editJobIdInput.value = String(job.id);
